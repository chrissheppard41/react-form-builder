...
describe('Reducer tests', () => {
  ...
  it('Should call the add input function', () => {
    ...
    expect(addInput({lorem: 'ipsum'})).toEqual({type: Actions.ADD_INPUT, payload: {InputData: {lorem: 'ipsum'}}});
  });

  it('Should call the move input function but shouldn\'t move the order', () => {
    const {moveInput} = jooks.run();

    expect(moveInput(0, 0)).toEqual({type: Actions.ALL_INPUT, payload: {InputData: inputs}});
  });

  it('Should move the input from position 0 to position 1', () => {
    const {moveInput} = jooks.run();

    expect(moveInput(0, 1)).toEqual({type: Actions.ALL_INPUT, payload: {InputData: {
          idTwo: inputs.idTwo,
          idOne: inputs.idOne,
  ...
  it('Should save an input', () => {
    ...
    expect(save({lorem: 'ipsum'})).toEqual({type: Actions.SAVE, payload: {data: {lorem: 'ipsum'}}});
  });

  it('Should save the id of the input field', () => {
    const {editInput} = jooks.run();

    expect(editInput('ipsum')).toEqual({type: Actions.EDIT_INPUT, payload: {id: 'ipsum'}});
  });

  it('Should delete the id of the input field', () => {
    const {deleteInput} = jooks.run();

    expect(deleteInput('ipsum')).toEqual({type: Actions.DELETE_INPUT, payload: {id: 'ipsum'}});
  });
  ...
  it('Should set the panel name', () => {
    ...
    expect(setPanel('lorem')).toEqual({type: Actions.SET_PANEL, payload: {panelName: 'lorem'}});
  });

  it('Should add validation types', () => {
    const {addValidation} = jooks.run();

    expect(addValidation('lorems', 'ipsum')).toEqual({type: Actions.SET_FORM_INPUT_ERROR, payload: {id: 'lorems', validationRule: 'ipsum'}});
  });
  ...
  it('Should delete validation types', () => {
    ...
    expect(removeValidation('lorem', 'ipsum')).toEqual({type: Actions.REMOVE_FORM_INPUT, payload: {id: 'lorem', validationRule: 'ipsum'}});
  });
  ...
  it('Should add a modal with a name and data', () => {
    ...
    expect(manageModals('lorem', data)).toEqual({type: Actions.MANAGE_MODALS, payload: {modalName: 'lorem', data}});
  });

  it('Should enable the children', () => {
    const {enableChildren} = jooks.run();

    expect(enableChildren('idOne', 'value')).toEqual({type: Actions.ENABLE_CHILDREN, payload: {id: 'idOne', enable: true}});
  });

  it('Should disable the children', () => {
    const {enableChildren} = jooks.run();

    expect(enableChildren('idTwo', '')).toEqual({type: Actions.ENABLE_CHILDREN, payload: {id: 'idTwo', enable: false}});
  });
