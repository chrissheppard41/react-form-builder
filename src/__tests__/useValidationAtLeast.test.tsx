...
const atLeastCheck = (values: string[], messageCompare: string, count: number, enabledCompare: boolean) => {
    const jooks = init(() => useValidationAtLeast({
        atLeast: {
            enabled: enabledCompare,
            message: 'lorem ipsum',
            count
        }
    }, values, 'id', addValidation, removeValidation));
...
describe('useValidationEmail hook tests', () => {
    it('Should not recognise what this validation is and no nothing', () => {
        const jooks = init(() => useValidationAtLeast({}, [], '', addValidation, removeValidation));
        const {atLeastMessage, atLeast} = jooks.run();
